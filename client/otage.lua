...
local hostageAllowedWeapons = Config.armeotage
...
function otage()
	...
	if not canotage then 
		drawNativeNotification(Config.otage.messagepasarme)
	end
	...
	if not holdingHostageInProgress and canotage then		
		...
		if closestPlayer ~= -1 and closestPlayer ~= nil then
			...
			drawNativeNotification(Config.otage.messagepersonne)
		end 
	end
	canotage = false 
end 
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,Config.otage.message)
